...
public:
  ...
  // Disable copying to keep ownership of the internal array simple.
...
Queue<T>::Queue(std::size_t capacity)
    : data(new T[capacity]), cap(capacity), front_index(-1), rear_index(-1),
      count(0) {}
...
template <typename T> bool Queue<T>::is_full() const {
  return rear_index == static_cast<int>(cap) - 1;
}
